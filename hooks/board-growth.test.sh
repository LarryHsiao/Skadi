#!/bin/bash
# Offline tests for board-growth.sh. Uses the seams BOARD_GROWTH_LINE (inject the
# headline, skip BigQuery) and BOARD_DIR (temp folder). Run: bash board-growth.test.sh
set -uo pipefail
...
d=$(tmpdir)
BOARD_DIR="$d" BOARD_GROWTH_LINE="$LINE" bash "$GROWTH" >/dev/null 2>&1
...
d=$(tmpdir)
BOARD_DIR="$d" BOARD_GROWTH_LINE="no numbers here" bash "$GROWTH" >/dev/null 2>&1
rc=$?
