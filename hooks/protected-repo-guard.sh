...
try:
    ...
    # An unbalanced quote. The common source is a heredoc body bearing an
    # apostrophe, since shlex knows nothing of heredoc syntax and reads it as
    # an unterminated quote. Say so rather than swallow it: an empty token
    # list is indistinguishable from a clean command, and the guard would go
    # mute for exactly the knotty commands most worth a word.
    print("ERROR:unparsable")
PYEOF
)
