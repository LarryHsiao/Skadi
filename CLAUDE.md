...
**A channel with a machine drain must not be named after a repo.** `handoff-autosub.sh` joins every session to a channel named for the repo it stands in, and a channel is a queue with one consumer per message — so a drain whose channel carries a repo's name gains a rival consumer in every session opened there. Name it for the traffic instead, and watch it with `read`, which prints without consuming, never `subscribe`, which enrols you in the consuming poll. The `Worklog` section below records what this cost the one time it happened.

Found late, the remedy is to rename the channel, not to leave it: there is no `unsubscribe` verb, so a session that has already auto-joined competes until it ends. The rule binds only channels with a drain — the per-repo ones have none, the peers there are equals, and first-come-first-served is the right reading of a baton meant for whoever is at the bench.
...
**Prefer the profile's external runner over an Agent-tool spawn.** `~/.skadi/profiles/<profile>/subagent-runner.md` — the profile from `SKADI_PROFILE`, so a work profile may run on Codex while the personal one keeps to the Agent tool — names a CLI to hand delegated work to: line 1 is its non-interactive command (prompt on stdin, `{model}` for the slug), the lines after map the roster's tiers (`mechanical=`, `default=`, `strong=`) to that runner's models; `~/.claude/hooks/subagent-runner.sh show` prints it. When it stands, dispatch through it — the same triad, the same report contract, only the transport differs — and fall back to the Agent tool only for what it cannot do: a task needing this conversation's live context, or Claude-only tooling. When it reads `none`, the Agent tool is the path. When it is absent (`unset`), ask the user once at the first dispatch that would want it — `subagent-runner.sh init` seeds it from a runner found on `PATH`, `init --none` records the refusal — and never ask again once written. `install.sh` never writes it; it prints a hint for each profile left unset, because which runner a profile uses, and whether spare quota stands behind it, is a choice about that profile and that machine, not a fact the config can know.
...
**Sending an entry.** When a turn will report work as done or complete — the same moment `Compliance Review` above fires — send a fuller paragraph summary via `/handoff send worklog-inbox <summary>`, opening it with that header line, before the final report. Correct a default that reads false, but never skip the entry to dodge the choice. This is best-effort: a failed send is named plainly in the summary, never a reason to withhold or delay the actual completion report.

**A verification step is not itself a completed task.** Launching the app, running a test, kicking a build — these serve the task in progress; they do not conclude it. None earns its own worklog entry. The judgment is which turn actually closes a piece of work: only that turn sends, and its summary may name the verification step as evidence ("verified via a launch on the emulator") without the step having triggered a send on its own account. A worklog padded with one line per intermediate action is a worklog no longer worth reading.

**Why the channel is `worklog-inbox` and not `worklog`.** A handoff channel is a **queue with one consumer per message** — whichever subscriber wakes first takes it, and the delete is immediate — while `handoff-autosub.sh` joins every session to a channel named for the repo it stands in. So while entries flowed to `worklog`, any session opened in the worklog repo became a second mouth on the same queue and ate entries the sync script was meant to file. That is not hypothetical: two entries were lost that way and recovered only because the session that swallowed them still had them in its transcript. No repo is named `worklog-inbox`, so nothing auto-joins it and the sync identity drinks alone.
