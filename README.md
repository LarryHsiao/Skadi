...
| `handbook/` | A browsable HTML handbook — the cover plus six chapters: work-loop, plan→forge→review, the Rúmil road, the untracked road, the fellowship of skills, and the standing machinery |
...
- `/commit` — Generate a commit message from the diff and commit directly, no approval prompt; pass `--confirm` to ask first, `--push` to push after the commit lands
...
- `/findegil` — Rewrite a set of notes into a blog post that reads as the author thinking out loud, not a press release: short average sentence length with wide variance, an oblique opening, uncertainty left unresolved. Carries a separate English and Traditional Chinese rule, each rebuilt for its language rather than translated. Invents nothing — no anecdote, feeling, or number absent from the notes — and names where the notes ran thin instead of filling the gap. Renders to chat; never writes a file
- `/pengolodh` — The hand tool for a repo's mechanism cache: `file:line`-anchored facts (entry points, live-vs-dead modules, seams, invariants, conventions in force) that grep cannot answer on its own. Built opportunistically as a side effect of ordinary work, never by a deliberate scan, and delivered automatically into every session's context at start. `verify` repairs a moved line number and reports what merely went stale or missing; `gc` deletes what verify confirms dead; bare `/pengolodh` prints the cache. See `docs/workflow/mechanism-cache.md`
...
- `/vilya` — Hold a protocol-less local server — a Vite or webpack dev server, a Zola or Hugo preview, an API mock — alive across turns and sessions, register its address as a clickable row on the statusline, and answer `ready` from the server's own log, so a caller waits on a signal rather than sleeping and hoping. `start` / `ready` / `status` / `stop` / `log`, one server per (project, name). Any caller may use it — you in chat, `/feanor` between mends, `/manwe` before a shot. It boots no browser and navigates nothing
- `/narya` — Keep a Flutter app alive on a booted emulator/simulator behind a standing `flutter run --machine` daemon, so a source edit reaches the running app in about a second rather than a 60–90s rebuild — and without losing the screen you navigated to. `start` pays the build once; `reload` / `restart` / `status` / `stop` / `log` poke the daemon thereafter. Any caller may use it — you in chat, `/feanor` between mends, `/manwe` before a shot. It boots no device and drives no tap; navigation stays yours
...
- `/board` — A standing situation board served in the browser: the tickets in progress (tracker status plus an AC rate derived from subtask completion), the metis growth pulse, a Stability tile (crash-free users % per app, chosen from a live dropdown), and an Attention band answering "what awaits me" (open GitLab MRs, open GitHub PRs, Jira tickets that moved in the last day), each a tile following a JSON channel file on disk. `add` / `remove` manage ticket channels; `refresh` re-fetches every ticket, the growth numbers, and the Attention surfaces, and with `--stability-scrape` sweeps each bound app's crash-free number, naming the ones needing a Firebase-console read; `stability-write` persists what that read found; `attention <mrs|prs|jira> [--clear]` refreshes one Attention surface directly, or clears a stale one; `list` prints the channels. Read-only against Jira, GitHub, GitLab, BigQuery, and the Crashlytics/GA4 exports
...
- **vilya** — The standing hold on a protocol-less local server for `/vilya`: `nohup` and a pid file, no pipe apparatus, because a dev server reads no stdin and speaks no protocol. Owning the process is what makes its log reachable, and `ready` waits on that log — from a byte offset, so an announcement made before the edit does not count
- **flutter-daemon** — The standing `flutter run --machine` session behind a named pipe for `/narya`: `start` pays the build once, then `reload` / `restart` reach the running app in about a second. A persistent writer holds the pipe open, without which the daemon takes the stdin EOF that kills a backgrounded `flutter run`
- **window-register** — Register or unregister one entry in the generic standing-windows registry the statusline reads: any script starting a local server a person might want to click into drops a two-line file (url, label) under `~/.skadi/windows`; a file whose server has gone silent is pruned on sight, so calling `unregister` on a clean shutdown is a courtesy, not a requirement
...
- **worklog-reminder** — Re-inject the worklog entry owed alongside Compliance Review with every prompt, supplying the `[<category>] <project>` header's two defaults so the label is a choice made rather than a question forgotten
