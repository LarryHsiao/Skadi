...
- **A test that can't reproduce the bug is a root-cause signal, not a license to substitute.** When a test fails to reproduce a target bug after a few input-guessing attempts, instrument and measure the real system state instead of continuing to guess. Falling back to an isolated/synthetic fixture that proves only the *fix's mechanism* — no longer exercising the real component — is a scope decision for the human, never a silent pivot: it decouples the test from the bug it claims to guard, and can hide unrelated regressions the fix itself introduces.
