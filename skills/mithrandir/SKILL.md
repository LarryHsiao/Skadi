...
- **Security** — injection (SQL, command, template), secrets in code or logs, auth bypass, broken access control (IDOR — a handler that trusts a client-supplied ID without checking the caller owns or may access that resource; forced browsing — a route reachable by knowing its exact URL/path with no permission gate in front of it; missing function-level authorization — an endpoint that authenticates the caller but never checks whether that caller may perform this specific action), missing input validation, deserialisation hazards, broken cryptography. Probe: does the change widen any surface that a hostile input could reach, or add/alter a route, handler, or resource-access path without a matching permission check? *Fires when the diff touches user input handling, network code, auth/session, file IO, command execution, crypto, templating, or adds/modifies a route, endpoint, controller, or resource-access handler. Skips on pure refactors with no surface change.*
...
**Security** <gauge>  <tier> — <evidence clause, ≤ 25 words>            ← conditional; render only if the diff touches surface (input, network, auth, IO, exec, crypto, templating, routes/endpoints)
