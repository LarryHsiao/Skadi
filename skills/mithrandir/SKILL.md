...
**Worth keeping section.** Optional. Render only when there are concrete bright spots — specific decisions, named files or patterns the work has done well. Heading is `## Worth keeping` in both modes. Each bullet ≤ 50 words; lead with a backticked file path or topic. Praise must be grounded in a specific decision the diff makes; do not pad with generic affirmations. Bright spots include **defensive moves**, not only well-built additions — a small change that *averts* a regression, race, or edge case (a `labelPadding: zero` that stops a label wrapping). These are the easiest to miss, reading as trivial params yet resting on non-obvious understanding; name the risk averted and why the fix is not obvious. If there is nothing genuine to keep, omit the section entirely — the praise must mean something or it loses weight.
